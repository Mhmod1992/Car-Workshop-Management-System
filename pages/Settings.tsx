...
const PlatePreviewSettings: React.FC = () => {
    ...
    const handleSave = async () => {
        await updateSettings({ ...settings, platePreviewSettings: currentSettings });
        addNotification({ title: 'نجاح', message: 'تم حفظ الإعدادات بنجاح!', type: 'success' });
    };
...
const InspectionSettingsTabs: React.FC = () => {
    const { customFindingCategories, addCustomFindingCategory, updateCustomFindingCategory, deleteCustomFindingCategory, predefinedFindings, deletePredefinedFinding, showConfirmModal, addNotification } = useAppContext();
    ...
    const handleDeleteCategory = (categoryId: string) => {
        showConfirmModal({
            ...
            onConfirm: async () => {
                const findingsToDelete = predefinedFindings.filter(f => f.categoryId === categoryId);
                await Promise.all(findingsToDelete.map(f => deletePredefinedFinding(f.id)));
                await deleteCustomFindingCategory(categoryId);
            }
        });
    };
    ...
    const handleSaveCategory = async () => {
        ...
        if (isEditingCategory) {
            await updateCustomFindingCategory(currentCategory as CustomFindingCategory);
        } else {
            const newCategory = { id: uuidv4(), name: currentCategory.name.trim() };
            await addCustomFindingCategory(newCategory);
