...
const FillRequestForm: React.FC = () => {
    const { 
        ...
        setSelectedRequestId, addNotification, updateRequest
    } = useAppContext();
    ...
    const handleSave = async () => {
        const updatedRequest = { ...request, generalNotes, structuredFindings, categoryNotes };
        await updateRequest(updatedRequest);
        addNotification({ title: 'نجاح', message: 'تم حفظ البيانات بنجاح.', type: 'success' });
    };

    const handleComplete = async () => {
        const updatedRequest = { ...request, generalNotes, structuredFindings, categoryNotes, status: RequestStatus.Completed };
        await updateRequest(updatedRequest);
