...
// Fix: Import 'initialSettings' to be used in the handleReset function.
...
const PrintReportPage: React.FC = () => {
    const { 
        settings, setPage, setSettingsPage, 
        customFindingCategories, predefinedFindings, 
        requests, selectedRequestId, clients, cars, 
        carMakes, carModels, inspectionTypes, employees 
    } = useAppContext();
    ...
    const memoizedData = useMemo(() => {
        ...
        const carData = cars.find(c => c.id === currentRequest.carId);
        const clientData = clients.find(c => c.id === currentRequest.clientId);
        const carMakeData = carData ? carMakes.find(m => m.id === carData.makeId) : undefined;
        const carModelData = carData ? carModels.find(m => m.id === carData.modelId) : undefined;
        const inspectionTypeData = inspectionTypes.find(i => i.id === currentRequest.inspectionTypeId);
        const employeeData = employees.find(e => e.id === currentRequest.employeeId);

        if (!clientData || !carData || !carMakeData || !carModelData || !inspectionTypeData || !employeeData) {
            console.error("Incomplete data for request:", currentRequest.id);
            return null;
        }
    ...
    }, [selectedRequestId, requests, clients, cars, carMakes, carModels, inspectionTypes, employees]);
    ...
    const dataMap = useMemo(() => {
        ...
        const { request, client, car, carMake, carModel, inspectionType, employee } = memoizedData;
    ...
    const renderBlockPreview = useCallback((block: ReportBlock) => {
        ...
        const { request, inspectionType } = memoizedData;
    ...
    }, [settings, memoizedData, dataMap, customFindingCategories, predefinedFindings]);

    if (!memoizedData) {
        return (
            <div className="bg-slate-100 min-h-screen flex flex-col items-center justify-center text-center">
                 <div className="no-print p-4 bg-white shadow-md flex justify-center items-center gap-4 sticky top-0 z-10 flex-shrink-0 w-full">
                ...
                <div className="p-8">
                    <h2 className="text-2xl font-bold text-gray-700 mb-4">لا يمكن عرض التقرير</h2>
                    <p className="text-gray-500">الرجاء تحديد طلب من قائمة الطلبات أولاً لعرض تقريره.</p>
                </div>
            </div>
        );
    }
...
export default memo(PrintReportPage);