...
const NewRequestForm: React.FC = () => {
    const {
        setPage, requests, clients, carMakes, carModels, inspectionTypes, brokers, settings, customFindingCategories,
        addNotification, addClient, addCar, addRequest
    } = useAppContext();
    ...
    const clientForPrint = newRequest ? clients.find(c => c.id === newRequest.clientId) : null;
    const carForPrint = newRequest ? { id: newRequest.carId, makeId: carMakeId, modelId: carModelId, year: carYear, plateNumber: useChassisNumber ? chassisNumber : `${plateChars} ${plateNums}` } : null;
