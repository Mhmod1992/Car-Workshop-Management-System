...
const BrokersManagement: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        if (isEditing) {
            await updateBroker(brokerToSave as Broker);
        } else {
    ...
    const handleToggleStatus = async (id: string) => {
        const brokerToUpdate = brokers.find(b => b.id === id);
        if (brokerToUpdate) {
            await updateBroker({ ...brokerToUpdate, isActive: !brokerToUpdate.isActive });
        }
    };
