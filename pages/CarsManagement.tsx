...
const CarsManagement: React.FC = () => {
    const { 
        carMakes, carModels, showConfirmModal, addNotification,
        addCarMake, updateCarMake, deleteCarMake, addBatchCarMakes,
        addCarModel, updateCarModel, deleteCarModel, addBatchCarModels
    } = useAppContext();
    ...
    const handleDeleteMake = (id: string) => {
        showConfirmModal({
            ...
            onConfirm: async () => {
                const modelsToDelete = carModels.filter(m => m.makeId === id);
                // Use Promise.all for batch deletion if supported, otherwise loop
                await Promise.all(modelsToDelete.map(model => deleteCarModel(model.id)));
    ...
    const handleSaveMake = async () => {
        ...
        if (isEditingMake) {
            ...
            await addCarMake({ ...currentMake, id: uuidv4() } as CarMake);
        }
        setIsMakeModalOpen(false);
    };

    const handleSaveBatchMakes = async () => {
        ...
        if (newMakes.length > 0) {
           await addBatchCarMakes(newMakes);
        }
    ...
    const handleSaveModel = async () => {
        ...
        if (isEditingModel) {
            ...
            await addCarModel({ ...currentModel, id: uuidv4() } as CarModel);
        }
    ...
    const handleSaveBatchModels = async () => {
        ...
        if (newModels.length > 0) {
            await addBatchCarModels(newModels);
        }
